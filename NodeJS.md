...
- 사용방법
  ...
  2. 적용하고 싶은 pug파일에다가 ```include 경로/video``` 를 하고 mixin을 이용하고 싶은 위치에 ```+hi(n)``` 코드를 작성한다.(여기서 hi는 mixin의 이름이며 앞에 +를 꼭 붙여야 하고 n은 인자이며 hi에서 사용하는 매개변수에 알맞게 넣어줘야 한다. 또한 extends한 파일에 block이 있을 경우 mixin은 block 안에서 사용해줘야 한다.)
...
- 미들웨어 사용
  - GET
    - ```라우터명.get("/경로", 미들웨어명, 컨트롤러명);```
  - POST
    - ```라우터명.post("/경로", 미들웨어명, 컨트롤러명);```
  - GET, POST 한번에
    - ```라우터명.route("/경로").all(미들웨어명).get(컨트롤러명).post(컨트롤러명);``` get과 post에 한번에 미들웨어를 적용하기 위해서는 ```all```을 사용할 수 있다.
- 컨트롤러에서의 사용
  - POST
    - ```(request객체).body```를 통해서 폼 안의 값들에 접근하여 사용할 수 있다.
...
- 렌더링하지 않고 상태코드만 보낼 경우에는
  ```javascript
  return res.sendStatus(400);
  ```
  위의 코드처럼 sendStatus를 이용한다. status는 render 하기 전에 상태코드를 정하는 것이고 sendStatus는 상태코드를 보내고 연결을 끊는 것이기 때문이다.
...
- 사용방법
  ...
  ```
  위와 같이 하면 컨트롤러에서 render를 할 때 변수를 넘겨주지 않아도 pug파일에서 변수를 이용할 수 있게 된다. 변수를 이용할 때 ```respond객체.locals.변수명``` 과 같이 쓸 필요 없고 그냥 ```변수명```만 적어주면 된다.
...
- 사용방법 2가지
  ...
  - 위의 방식대로 하면 파일마다 코드를 작성해줘야하는 번거로움이 있다. 따라서 좀 더 간편하게 사용하고 싶으면 앱을 시작하는 가장 앞부분(package.json파일의 "scripts"에서 찾을 수 있다.)에 ```import "dotenv/config";``` 라는 코드를 작성한다.
