...
- 코드 작성방법
  ...
  - footer와 같이 여러 페이지에 동일하게 적용되는 부분을 페이지마다 작성하기는 너무 번거롭다. 따라서 이런 경우에는 따로 파일을 만들어 내용을 작성하고 이 내용을 적용하고 싶은 페이지의 pug파일에 ```include 파일명.pug``` 라고 해주면 된다.
  파일명을 적을 때 경로를 잘 적어줘야 한다.
